...
@php
    ...
    $base = "ui-button ui-button-{$variant} ui-button-{$size}";
@endphp

@if ($href && $disabled)
    <span {{ $attributes->except(['class', 'href', 'target'])->merge(['class' => trim($base.' '.$classes), 'role' => 'button', 'aria-disabled' => 'true', 'tabindex' => '-1']) }}>
        @if ($icon)
            <flux:icon :name="$icon" variant="mini" class="ui-button-icon" />
        @endif
        <span>{{ $slot }}</span>
        @if ($iconEnd)
            <flux:icon :name="$iconEnd" variant="mini" class="ui-button-icon" />
        @endif
    </span>
@elseif ($href)
