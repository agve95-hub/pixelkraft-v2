...
export default function ProductCreate({ site }: { site: Site }) {
    const { data, setData, post, processing, errors } = useForm({
        name: '', description: '', price: '', currency: 'EUR', status: 'draft',
    });

    return (
        <AppLayout title="New product">
            <Head title={`New product — ${site.name}`} />
            <form onSubmit={(e) => { e.preventDefault(); post(`/dashboard/sites/${site.id}/products`); }}>
