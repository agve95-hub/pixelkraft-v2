...
import { type PageProps, type NavSite } from '@/types';
...
function siteMenuItems(site: NavSite) {
    return [
        { icon: Mail, label: 'Inbox', href: `/dashboard/sites/${site.id}/inbox`, badge: site.unread_inbox_count },
        { icon: ClipboardList, label: 'Reports', href: `/dashboard/sites/${site.id}/reports` },
        { icon: Megaphone, label: 'Campaigns', href: `/dashboard/sites/${site.id}/campaigns` },
        { icon: Banknote, label: 'Expenses', href: `/dashboard/sites/${site.id}/expenses` },
        { icon: FileText, label: 'Invoices', href: `/dashboard/sites/${site.id}/invoices`, badge: site.unpaid_invoices_count },
        { icon: Clock, label: 'Reminders', href: `/dashboard/sites/${site.id}/reminders`, badge: site.overdue_reminders_count, badgeVariant: 'warning' as const },
        { icon: BarChart, label: 'Analytics', href: `/dashboard/sites/${site.id}/analytics` },
        { icon: ShieldCheck, label: 'Maintenance', href: `/dashboard/sites/${site.id}/maintenance` },
        { icon: Image, label: 'Media', href: `/dashboard/sites/${site.id}/files` },
    ];
}
    ...
    const SidebarContent = () => (
        <div className="flex h-full flex-col">
            ...
            <div className="flex items-center justify-between border-b border-zinc-700 px-4 py-3">
                <Link href="/dashboard" className="flex items-center gap-2.5 text-[15px] font-semibold tracking-tight text-white no-underline">
                        ...
                        return (
                            <div key={site.id}>
                                <button
                                    ...
                                    className={cn(
                                        'flex w-full items-center gap-2.5 rounded-md px-2.5 py-1.5 text-sm text-zinc-300 transition-colors hover:bg-zinc-800 hover:text-zinc-100',
                                        siteActive && 'text-zinc-100',
                                    )}
                                >
                                    <span className={cn('size-2 rounded-full shrink-0', deployStatusColor(site.deploy_status))} />
            ...
            <div className="border-t border-zinc-700 p-2">
                ...
                >
                    ...
                    <span>Search</span>
                    <span className="ml-auto inline-flex items-center rounded bg-white/[0.07] px-1 py-0.5 font-mono text-[9px] text-zinc-400">⌘K</span>
                </button>
                ...
                {user && (
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <button className="mt-2 flex w-full items-center gap-2.5 rounded-md px-2 py-1.5 text-sm hover:bg-zinc-800 transition-colors">
    ...
    return (
        <TooltipProvider>
            <div className="flex min-h-screen">
                {/* Desktop sidebar */}
                <aside className="hidden lg:flex w-64 shrink-0 flex-col border-r border-zinc-700 bg-zinc-900">
                    <SidebarContent />
                </aside>
